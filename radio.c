...
uint8_t Selektion = 30;// Der aktuell selektierte Menueintrag
bool IfSel = false;// ist die Selektion angewaehlt?
...
void setMenuReg(uint8_t nr, uint16_t data){
...
void paintRahmen(uint16_t left, uint16_t top, uint16_t width, uint16_t height){
	uint16_t area[] = {left, top, left+width, top+height};
	paintRect(area, 0x0000);//leeren
	area[3] = top;
	paintRect(area, 0x835F);// oben
	area[3] = top+height; area[1] = top+height;
	paintRect(area, 0x835F);// unten
	area[1] = top; area[3] = top+height; area[2] = left;
	paintRect(area, 0x835F);// links
	area[0] = left+width; area[2] = left+width;
	paintRect(area, 0x835F);// rechts
}
void paintBoolMenu(bool org, bool sel){
	paintRahmen(140, 0, 40, 40);
	char text[4];
	uint16_t pos[] = {145, 4};
...
void paintOptMenu(uint8_t org, uint8_t sel){
	...
	paintRahmen(left, top, width, height);
	//paint options
...
void takeSel(uint8_t nr){
	if(nr == 13){
		...
		else if(Selektion < 21 || Selektion == 28){
...
int main() {
	...
	while (true) {
		uint8_t button = get_Button();
		if(button < 13){ if(! IfSel) changeSel(button); }
		else if(button < 14){ if(! IfSel) takeSel(button); }
		busy_wait_ms(1);
	}
}
